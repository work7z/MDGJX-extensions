...
exports.fofo = {
    ...
    pushData: function (databases, data) {
        var db = mdgjx.db.get(databases);
        if (db) {
            mdgjx.db.put({ _id: databases, data: data, _rev: db._rev });
        } else {
            mdgjx.db.put({ _id: databases, data: data });
        }
    },
    ...
    showChangeLog: function () {
        ...
        fs.readFile(path.join(__dirname, 'CHANGELOG.md'), { encoding: 'utf8' }, (err, data) => {
            if (data) mdgjx.ubrowser.goto(data, '更新日志').run({
                width: 1080,
                height: 860
            })
        })
    ...
    isRunningAtFirstTime: function () {
        try {
            var historyVersion = mdgjx.db.get('plugin').data.version
