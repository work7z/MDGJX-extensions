...
showOptions = async () => {
    ...
    var currentFts = mdgjx.getFeatures();
...
$("#options").on('change', 'input[type=checkbox]', async function () {
    ...
    if (num == '1') {
        ...
        if ($(this).prop('checked')) {
            mdgjx.setFeature(allFts[code].features);
            $(`#${id}_2`).prop('disabled', false);
        } else {
            mdgjx.removeFeature(code);
        ...
        mdgjx.setFeature(featureConf);
    }
});

// 底部功能按钮
$("#options").on('click', '.footBtn', function () {
    mdgjx.setSubInputValue('');
    switch ($(this).attr('id')) {
        case 'add':
...
$("#options").on('click', '.editBtn', async function () {
    ...
    if (window.manualVars.defaultPage) {
        ...
        $.get(url, content => {
            mdgjx.db.put({ _id: code, data: content.entries });
            showOptions();
        })
    } else {
        var data = mdgjx.db.get("customFts").data[code];
        showCustomize();
...
$("#options").on('click', '.delBtn', function () {
    ...
    if (window.manualVars.defaultPage) {
        ...
        delete data[code];
        mdgjx.removeFeature(code);
        mdgjx.db.put({ _id: "customFts", data: data, _rev: db._rev }); 
    }
    showOptions();
})
...
$("#options").on('click', '.saveBtn', async function () {
    ...
    if (code in allFts && $("#code").prop('disabled') == false) {
        ...
        if (db) {
            ...
            mdgjx.db.put({ _id: "customFts", data: data, _rev: rev });
        } else {
            mdgjx.db.put({ _id: "customFts", data: pushData });
        }
        showOptions();
    }
})
