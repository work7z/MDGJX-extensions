...
showList = (text, index, listnum) => {
    ...
    let num = $(".info").length
    mdgjx.setExpendHeight(num > 11 ? 550 : 50 * num);
...
// 列表界面子输入框
mainlistSubInput = () => {
    mdgjx.removeSubInput();
    mdgjx.setSubInput(({ text }) => {
        // 列表搜索
...
// 进入插件
mdgjx.onPluginEnter(async ({ code, type, payload }) => {
    ...
    if (code == 'options') {
        ...
        try {
            ...
            if (type == 'regex') {
                mdgjx.setSubInputValue(payload)
    ...
    switch (e.keyCode) {
        ...
        case 83:
            if ($('#mainlist').is(':hidden') && $("#manual").is(":visible")) {
                ...
                if (text) {
                    mdgjx.redirect('备忘', { 'type': 'text', 'data': text })
                }
            }
            break;
        // 划词翻译
        case 84:
            if ($('#mainlist').is(':hidden') && $("#manual").is(":visible")) {
                ...
                if (text) {
                    // if (/[\u4e00-\u9fa5]/.test(text)){
                        // mdgjx.showNotification('中文你还看不懂嘛！', clickFeatureCode = null, silent = true)
                    // } else {
